...
theta_init = pi/4;
sampling_time = 0.05;
[ctrb_matrix, state_transition_matrix] = get_ctrb_and_state_transition_matrices_unicycle(mode_sequence_omega,theta_init,sampling_time);
% Initial location
x_init = 0;
y_init = 0;
...
for time_of_interest = 1:length(mode_sequence_omega)+1
    ...
    elapsed_time(time_of_interest)=toc(timerVal);
     [mu_trajectory{time_of_interest}, sigma_obstacle{time_of_interest}] =...
      get_FSRPD_mean_and_covariance_matrix(ctrb_matrix(:,1:time_of_interest),...
...
disp(sprintf('Mean and Covariance matrices\n(99%% confidence ellipses)'));
