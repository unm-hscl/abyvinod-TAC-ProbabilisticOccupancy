...
%                 target_support_position(t_indx_plus1));
    ...
    % ensures that target_relv_states (added on top of it) is of the correct time
    % snapshot
    if t_indx_plus1 > 1
        relv_indx = target_sys.state_dim * (t_indx_plus1-2) + target_relv_states;
        target_mean_position = target_Z(relv_indx,:) * target_init_state+ ...
            target_H(relv_indx,:) * target_affine_vec + ...
            target_G(relv_indx,:) * target_concat_mean_dist_vec;
    else
    ...
    if init_feas_prob_catch_value(feas_list_indx) >= 1 - zero_catch_prob
        ...
        if ~use_gradient
            ...
            obj = @(x) -log(pursuit_occupy_fun_Levi(...
                x + catch_box, t_indx_plus1 - 1, target_sys, ...
                target_relv_states, target_init_state, ...
                target_affine_vec_slice, dist_delta, dist_peak, false));
        else
            % t_indx_plus1 -1 because t_indx_plus1 starts from 0 and here the
            % correct t_indx is required
            obj = @(x) obj_with_grad(x, catch_box, t_indx_plus1 - 1, ...
                target_sys, target_relv_states, target_init_state, ...
                target_affine_vec_slice, dist_delta, dist_peak);
        end
...
function [neg_log_prob, grad_after_log] = obj_with_grad(x, catch_box, t_indx,...
    ...
    [prob, grad] = pursuit_occupy_fun_Levi(x + catch_box, t_indx, target_sys,...
                    target_relv_states, target_init_state, target_affine_vec_slice, ...
